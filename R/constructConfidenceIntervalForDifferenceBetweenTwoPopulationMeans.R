#' @title constructConfidenceIntervalForDifferenceBetweenTwoPopulationMeans
...
#' @examples confidence_interval <- constructConfidenceIntervalForDifferenceBetweenTwoPopulationMeans(iris%>%filter(Species=="versicolor")%>%pull(Sepal.Length), iris%>%filter(Species=="setosa")%>%pull(Sepal.Length), 0.05)
#' @export

constructConfidenceIntervalForDifferenceBetweenTwoPopulationMeans <- function(data_for_sample_1, data_for_sample_2, significance_level) {
