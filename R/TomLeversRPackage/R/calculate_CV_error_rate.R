#' @title calculate_CV_error_rate
#' @description Calculates CV error rate
#' @param type_of_model The type of model. An element in the set {"Polynomial Regression", Logistic Regression"}.
#' @return CV_error_rate The error rate of Leave One Out Cross Validation
#' @examples CV_error_rate <- calculate_CV_error_rate(type_of_model = "LR", formula = Direction ~ Lag1 + Lag2, data_frame = ISLR2::Weekly)
...
calculate_CV_error_rate <- function(type_of_model, formula, data_frame, number_of_folds) {
 ...
 if (type_of_model == "Polynomial Regression") {
  ...
  CV_error_rate <- mean(vector_of_mean_squared_errors)
  return(CV_error_rate)
 } else if (type_of_model == "Logistic Regression") {
  vector_of_error_rates <- integer(0)
  for (i in 1:number_of_folds) {
   if (number_of_folds == 1) {
    training_data <- data_frame
    testing_data <- data_frame
   ...
   sum_of_indicators_of_error <- 0
   for (j in 1:number_of_test_observations) {
    if (vector_of_predicted_response_values[j] != testing_data[j, name_of_response]) {
     sum_of_indicators_of_error <- sum_of_indicators_of_error + 1
    }
   }
   error_rate <- sum_of_indicators_of_error / number_of_test_observations
   vector_of_error_rates <- append(vector_of_error_rates, error_rate)
  }
  CV_error_rate <- mean(vector_of_error_rates)
  return(CV_error_rate)
 } else {
