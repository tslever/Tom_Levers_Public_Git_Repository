...
summarize_linear_model <- function(linear_model) {
    ...
    regular_expression_for_decimal <- "-?\\d+(\\.\\d+)?"

    line_with_response_intercept <- the_summary[11]
    response_intercept <- str_extract(line_with_response_intercept, regular_expression_for_decimal)
    line_with_slope <- the_summary[12]
    slope <- str_extract(line_with_slope, regular_expression_for_decimal)
    ...
    multiple_and_adjusted_R_squared <- str_extract_all(line_with_coefficients_of_determination_R2, regular_expression_for_decimal)[[1]]
