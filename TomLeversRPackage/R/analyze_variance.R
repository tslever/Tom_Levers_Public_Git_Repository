...
analyze_variance <- function(linear_model) {
    ...
    DF_SSR_MSR_F0_and_P <- str_extract_all(line_with_SSR, regular_expression_for_decimal)[[1]]
    SSR <- as.double(DF_SSR_MSR_F0_and_P[2])
    line_with_SSRes <- analysis[6]
    DF_SSRes_and_MSRes <- str_extract_all(line_with_SSRes, regular_expression_for_decimal)[[1]]
    SSRes <- as.double(DF_SSRes_and_MSRes[2])
    SST <- SSR + SSRes
    line_with_SST <- paste("SST: ", SST, sep = "")
