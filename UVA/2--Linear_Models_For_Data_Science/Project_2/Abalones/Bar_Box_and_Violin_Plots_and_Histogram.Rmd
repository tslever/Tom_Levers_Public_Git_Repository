---
title: "Proposal Visualizations Not Related To Multiple Logistic Regression"
author: "Tom Lever"
date: 11/13/22
output:
...
library(dplyr)
library(ggplot2)
data_set <- read.csv('Data_Set--Abalone_Marine_Snails--With_Column_Names.csv', header = TRUE)
sex_and_proportion <-
    data_set %>% select(sex) %>%
    group_by(sex) %>%
    summarize(count = n()) %>%
...
ggplot(sex_and_mean_length, aes(x = sex, y = mean_length)) +
    geom_bar(stat = "identity", fill = "Blue", color = "Orange") +
    labs(
        title = "Distributions of Abalone Marine Snails",
        y = "mean length (mm)"
    ) +
    theme(
        plot.title = element_text(hjust = 0.5, size = 11),
