...
2. For adult Gentoo penguins near Palmer Station, what is the change in the predicted body mass (in grams)
   ...
   $$\left[\hat{\beta}_1 - t_{\alpha/2, \ df} \ SE\left(\hat{\beta}_1\right), \ \hat{\beta}_1 + t_{\alpha/2, \ df} \ SE\left(\hat{\beta}_1\right)\right]$$
    ...
    qt((1 + confidence_level)/2, degrees_of_freedom)
    # quantile t_{\alpha/2, df} for which probability that test statistic is greater
    # is half significance level
    ...
    Since the confidence interval $\left[44.668 \ \frac{g}{mm}, \ 64.577 \ \frac{g}{mm}\right]$ does not contain 0, we reject the null hypothesis $H_0: \beta = 0 \ \frac{g}{mm}$.
    We have sufficient evidence to support the alternate hypothesis $H_1: \beta \neq 0$.
    
    Since the confidence interval $\left[44.668 \ \frac{g}{mm}, \ 64.577 \ \frac{g}{mm}\right]$ contains $50 \ \frac{g}{mm}$, we fail to reject the null hypothesis $H_0: \beta = 50 \ \frac{g}{mm}$.
...
3. Conduct a hypothesis test to determine whether or not there is a linear association between body mass and flipper length
   ...
   $H_1: \beta_1 = 0$ that the slope of the linear model is not equal to 0.
   We have sufficient evidence to reject the null hypothesis if the test statistic $t_0 = 10.863$ is greater than $t_{\alpha/2, df} = 1.980$.
   Since $t_0 > t_{\alpha/2, df}$, we reject the null hypothesis. We have sufficient evidence to support the alternate hypothesis.
   $$t_0 = \frac{{\hat{\beta}_1}^2}{MS_{Res} / S_{xx}} = \frac{{\hat{\beta}_1}^2 S_{xx}}{MS_{Res}} = \frac{\hat{\beta}_1 S_{xy}}{MS_{Res}} = \frac{MS_R}{MS_{Res}} = F_0$$
   We have sufficient evidence to reject the null hypothesis if the probability $p < 2.2 \times 10^{-16}$ is less than $\alpha$.
   Since $p < \alpha$, we reject the null hypothesis. We have sufficient evidence to support the alternate hypothesis.
   
    ```{r, eval=TRUE, echo=TRUE, results="show", message=FALSE}
    test_statistic <- 10.863
    probability <- pt(test_statistic, degrees_of_freedom, lower.tail = FALSE)
    probability
...
5. Estimate the mean body mass for adult Gentoo penguins near Palmer Station with flipper lengths of $x_0 = 200 \ mm$.
   ...
   $$\left[ E\left(\hat{y} \ | \ x_0\right) - t_{\alpha/2, \ df} \ SE\left\{E\left(\hat{y} \ | \ x_0\right)\right\}, \ E\left(\hat{y} \ | \ x_0\right) + t_{\alpha/2, \ df} \ SE\left\{E\left(\hat{y} \ | \ x_0\right)\right\} \right]$$
...
6. Report the 95 percent prediction interval for the body mass of an adult Gentoo penguin with flipper length of $x_0 = 200 \ mm$.
   ...
   $$\left[ E\left(\hat{y} \ | \ x_0\right) - t_{\alpha/2, \ df} \ SE\left\{\hat{y} \ | \ x_0\right\}, \ E\left(\hat{y} \ | \ x_0\right) + t_{\alpha/2, \ df} \ SE\left\{\hat{y} \ | \ x_0\right\} \right]$$
...
7. A researcher hypothesizes that for adult Gentoo penguins near Palmer Station, the predicted body mass increases by more than $50 \ g$
   ...
   $H_1: \beta_1 > \beta_{10}$ if the test statistic $t_0$ is greater than $t_{\alpha/2, \ df} = 1.980$, where
   ...
   Since $t_0 = 0.919$ is less than $t_{\alpha/2, \ df} = 1.980$, we have insufficient evidence to reject that the null hypothesis
   ...
   We have sufficient evidence to reject the null hypothesis if the probability $p$ that a test statistic is greater than the magnitude of $t_0$,
   assuming the null hypothesis is true, is less than $\alpha$.
   
    ```{r, eval=TRUE, echo=TRUE, results="show", message=FALSE}
    pt(0.919, degrees_of_freedom, lower.tail = FALSE)
    ```
