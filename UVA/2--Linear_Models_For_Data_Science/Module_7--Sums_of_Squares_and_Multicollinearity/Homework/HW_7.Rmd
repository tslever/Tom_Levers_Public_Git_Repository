...
2.  You may only use R as a simple calculator or to find $p$-values or critical values. The data for this question come from $113$ hospitals.
    ...
    * $x_5$: $Beds$: The number of beds in the hospital.
    ...
    (b) Based on your answer in part 2a, carry out the appropriate hypothesis test to see if those predictors can be dropped from the multiple
        ...
        Since the R output describes an $F$ statistic for a Partial $F$ Test following an $F$ distribution with $DF_R = 5$ and $DF_{Res} = 107$
        degrees of freedom, a critical value for the Partial $F$ Test $F_c = 2.299$.
        ...
        Since this test statistic $F_0$ is less than the critical value $F_c = 2.299$, we have insufficient evidence to reject a null hypothesis 
        ...
        Since the $p$-value $p = 0.123$ corresponding to this $F$ statistic $F_0 = 1.782$ is greater than a significance level $\alpha = 0.05$,
    ...
    (c) Suppose we want to decide between two potential models:
        ...
        * Model 2 using $x_1$ and $x_2$ as the predictors for $InfctRsk$
        ...
        Since the test statistic $F_{0, \ w/o \ x_5} = 2.683$ is greater than the critical value $F_{c, \ w/o \ x_5} = 2.456$, we have sufficient
        evidence to reject a null hypothesis that the regression coefficients for the dropped predictors are 0. We have sufficient evidence to
        ...
        significant. The predictors $Age$ and $Census$ cannot be dropped simultaneously from the multiple linear model without the predictor 
        $Beds$.
        ...
        Since the $p$-value $p_{w/o \ x_5} = 0.0353$ is less than the significance level $\alpha = 0.05$, we have sufficient evidence to reject a
        null hypothesis that the regression coefficients for the dropped predictors are $0$. We have sufficient evidence to support an alternate
        hypothesis that a regression coefficient for a dropped predictor is not $0$. The dropped predictors are jointly significant. The 
...
3.  This question is based on a data set seen in Homework Set 4. Data from 55 college students are used to estimate a multiple regression model
    ...
    Since the R output describes an $F$ statistic for a Partial $F$ Test following an $F$ distribution with $DF_R = 2$ and $DF_{Res} = 52$
