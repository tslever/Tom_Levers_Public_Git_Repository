...
7. Create a bar chart similar to the bar chart in Part 6, but split by smoking status. Comment on this bar chart.
    ...
    ```{r, eval=TRUE, echo=TRUE, results="show", message=FALSE}
    smoking_status_and_GPA_category <-
        students_dataframe %>% select(Smoke, GPA.cat) %>% filter(!is.na(GPA.cat))
    colnames(smoking_status_and_GPA_category) <- c("smoking_status", "GPA_category")
    proportion_of_GPA_category_by_smoking_status <-
        smoking_status_and_GPA_category %>%
            group_by(smoking_status, GPA_category) %>%
            summarize(proportions_of_students = n()) %>%
            mutate(
                proportions_of_students = proportions_of_students / number_of_students
            )
    ggplot(
        ...
        labs(
            x = "smoking status",
            y = "proportions of students",
            title = "Proportions of GPA Category by Smoking Status"
        ) +
        theme(
            plot.title = element_text(hjust = 0.5),
            axis.text.x = element_text(angle = 0)
