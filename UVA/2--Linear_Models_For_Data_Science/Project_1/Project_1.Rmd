...
Price refers to the cost of each diamond per the BlueNile website.
...
as well as a plot of residuals versus predicted prices for the model,
...
for the residuals of the linear model of price versus weight, the tails of a probability vs. residuals plot / distribution are too light for this distribution to be considered normal.
...
```{r, eval=TRUE, echo=FALSE, results="show", message=FALSE, warning = FALSE, fig.width = 5.3, fig.height = 3.8, fig.align='center'}
acf(linear_model_of_price_and_weight$residuals, main = "ACF Value vs. Lag for Price vs. Weight")
```
...
5. The residuals of the linear model of price versus weight are normally distributed. This assumption is not met.
   ...
   the tails of a probability vs. residuals plot / distribution are too light.
...
We construct a linear model of log of price versus weight and present a scatterplot of residuals
...
data_set <- data_set %>% mutate(log_of_price = log(price))
ggplot() +
    geom_point(data = data_set, aes(x = weight, y = log_of_price), alpha = 0.2) +
    geom_smooth(data = data_set, aes(x = weight, y = log_of_price), method = "lm", se = FALSE) +
    labs(
        x = "weight (carat)",
        y = "log of price",
...
linear_model_of_log_of_price_and_weight <- lm(log_of_price ~ weight, data = data_set)

ggplot(
    data.frame(
        externally_studentized_residual = linear_model_of_log_of_price_and_weight$residuals,
        predicted_log_of_price = linear_model_of_log_of_price_and_weight$fitted.values
    ),
    aes(x = predicted_log_of_price, y = externally_studentized_residual)
) +
    geom_point(alpha = 0.2) +
    geom_hline(yintercept = 0, color = "red") +
    labs(
...
We present a scatterplot of residuals versus predicted log of price for the linear model of
...
```{r, eval=TRUE, echo=FALSE, results="show", message=FALSE, warning = FALSE, fig.width=3.3, fig.height=2.4}
data_set <- data_set %>% mutate(log_of_weight = log(weight))
ggplot() +
    geom_point(data = data_set, aes(x = log_of_weight, y = log_of_price), alpha = 0.2) +
    geom_smooth(data = data_set, aes(x = log_of_weight, y = log_of_price), method = "lm", se = FALSE) +
    labs(
        x = "log of weight",
        y = "log of price",
...
linear_model_of_log_of_price_and_log_of_weight <- lm(log_of_price ~ log_of_weight, data = data_set)
#print(summarize_linear_model(linear_model_of_log_of_price_and_log_of_weight))

ggplot(
    data.frame(
        externally_studentized_residual = linear_model_of_log_of_price_and_log_of_weight$residuals,
        predicted_log_of_price = linear_model_of_log_of_price_and_log_of_weight$fitted.values
    ),
    aes(x = predicted_log_of_price, y = externally_studentized_residual)
) +
    geom_point(alpha = 0.2) +
...
```{r, eval=TRUE, echo=FALSE, results="show", message=FALSE, warning = FALSE, fig.width = 5.3, fig.height = 3.8, fig.align='center'}
acf(linear_model_of_log_of_price_and_weight$residuals, main = "ACF Value vs. Lag for Transformed Linear Model")
```
...
5. The assumption that the residuals of the linear model of log of price versus log of weight are normally distributed
   ...
   a probability vs. residuals plot / distribution is not normal.

```{r, eval=TRUE, echo=FALSE, results="show", message=FALSE, warning = FALSE, fig.width = 5.3, fig.height = 3.8, fig.align='center'}
qqnorm(linear_model_of_log_of_price_and_weight$residuals)
qqline(linear_model_of_log_of_price_and_weight$residuals, col = "red")
```
