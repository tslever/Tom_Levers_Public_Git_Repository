...
13. This question should be answered using the `Weekly` data set, which is part of the `ISLR2` package.
    ...
    (c) Compute the confusion matrix and overall fraction of correct predictions. Explain what the confusion matrix is telling you about the types of mistakes made by logistic regression.
        ...
        vector_of_predicted_probabilities <- predict(
            object = LR_Model_With_Formula_Direction_Vs_Lag1_Lag2_Lag3_Lag4_Lag5_and_Volume,
            newdata = Weekly,
            type = "response"
        )
    ...
    (d) Now fit the logistic regression model using a training data period from 1990 to 2008, with Lag2 as the only predictor. Compute the confusion matrix and the overall fraction of correct predictions for the held out data (that is, the data from 2009 and 2010).
        ...
        LR_Model_With_Formula_Direction_Vs_Lag2
        # vector_of_predicted_probabilities is a vector of predicted probabilities that
        # an observation corresponds to the market having a positive return that week
        vector_of_predicted_probabilities <- predict(
            object = LR_Model_With_Formula_Direction_Vs_Lag2,
    ...
    (g) Repeat (d) using KNN with $K = 1$.
        ...
        ```{r}
        library(class)
        matrix_of_values_of_predictors_for_training <- matrix(
            data = Weekly_from_1990_to_2008_inclusive$Lag2
        )
        matrix_of_values_of_predictors_for_testing <- matrix(
            data = Weekly_from_2009_to_2010_inclusive$Lag2
        )
        vector_of_directions_for_training <- Weekly_from_1990_to_2008_inclusive$Direction
        set.seed(1)
        vector_of_directions <- knn(
            train = matrix_of_values_of_predictors_for_training,
            test = matrix_of_values_of_predictors_for_testing,
            cl = vector_of_directions_for_training,
            k = 1
        )
        confusion_matrix <-
            table(vector_of_directions, Weekly_from_2009_to_2010_inclusive$Direction)
    ...
    (j) Experiment with different combinations of predictors, including possible transformations and interactions, for each of the methods. Report the variables, method, and associated confusion matrix that appears to provide the best results on the held out data. Note that you should also experiment with values for $K$ in the KNN classifier.
        ...
        Assumptions for the following logistic model are met.
        $$Direction = ln\left[\frac{P(Y = 1 \ | \ Lag2)}{1 - P(Y = 1 \ | \ Lag2)}\right] = \beta_0 + \beta_1 \ Lag2 = 0.203 + 0.058 \ Lag2$$
        ...
        ```{r}
        library(TomLeversRPackage)
        calculate_fraction_of_correct_predictions(
            model = LR_Model_With_Formula_Direction_Vs_Lag1,
            data_set = Weekly_from_2009_to_2010_inclusive
        )
        calculate_fraction_of_correct_predictions(
            model = LR_Model_With_Formula_Direction_Vs_Lag2,
            data_set = Weekly_from_2009_to_2010_inclusive
        )
        calculate_fraction_of_correct_predictions(
            model = LR_Model_With_Formula_Direction_Vs_Lag1_And_Lag2,
            data_set = Weekly_from_2009_to_2010_inclusive
        )
        # Lag1:Lag2 denotes an interation term.
        # Lag1*Lag2 denotes Lag1 + Lag2 + Lag1:Lag2
        LR_Model_With_Formula_Direction_Vs_Interaction_Of_Lag1_And_Lag2 <- glm(
            formula = Direction ~ Lag1:Lag2,
            data = Weekly_from_1990_to_2008_inclusive,
            family = binomial
        )
        calculate_fraction_of_correct_predictions(
            model = LR_Model_With_Formula_Direction_Vs_Interaction_Of_Lag1_And_Lag2,
            data_set = Weekly_from_2009_to_2010_inclusive
        )
        ```
        
        Since our Logistic Regression and LDA models with formulas $Direction \sim Lag2$ have the same fraction of correct predictions, we assume our LDA models will behave similarly to our Logistic Regression models. We compare LDA models with formulas $Direction \sim Lag1$, $Direction \sim Lag2$, $Direction \sim Lag1 + Lag2$, and $Direction \sim Lag1 \ Lag2$.
        
        ```{r}
        LDA_Model_With_Formula_Direction_Vs_Lag1 <- lda(
            formula = Direction ~ Lag1,
            data = Weekly_from_1990_to_2008_inclusive
        )
        calculate_fraction_of_correct_predictions(
            model = LDA_Model_With_Formula_Direction_Vs_Lag1,
            data_set = Weekly_from_2009_to_2010_inclusive
        )
        calculate_fraction_of_correct_predictions(
            model = LDA_Model_With_Formula_Direction_Vs_Lag2,
            data_set = Weekly_from_2009_to_2010_inclusive
        )
        LDA_Model_With_Formula_Direction_Vs_Lag1_And_Lag2 <- lda(
            formula = Direction ~ Lag1 + Lag2,
            data = Weekly_from_1990_to_2008_inclusive
        )
        calculate_fraction_of_correct_predictions(
            model = LDA_Model_With_Formula_Direction_Vs_Lag1_And_Lag2,
            data_set = Weekly_from_2009_to_2010_inclusive
        )
        # Lag1:Lag2 denotes an interation term.
        # Lag1*Lag2 denotes Lag1 + Lag2 + Lag1:Lag2
        LDA_Model_With_Formula_Direction_Vs_Interaction_Of_Lag1_And_Lag2 <- lda(
            formula = Direction ~ Lag1:Lag2,
            data = Weekly_from_1990_to_2008_inclusive
        )
        calculate_fraction_of_correct_predictions(
            model = LDA_Model_With_Formula_Direction_Vs_Interaction_Of_Lag1_And_Lag2,
            data_set = Weekly_from_2009_to_2010_inclusive
        )
        ```
        ...
        ```{r}
        QDA_Model_With_Formula_Direction_Vs_Square_Of_Lag2 <- qda(
            formula = Direction ~ I(Lag2^2),
            data = Weekly_from_1990_to_2008_inclusive
        )
        calculate_fraction_of_correct_predictions(
            model = QDA_Model_With_Formula_Direction_Vs_Square_Of_Lag2,
            data_set = Weekly_from_2009_to_2010_inclusive
        )
        ```
        
        We experiment with different values of $K$ for our $K$ Nearest Neighbors classifier from part (g). We also experiment with different predictors for a KNN classifier. For our KNN classifier from part (g), increasing $K$ and decreasing variance by a factor of $187$ results in an increase in fraction of correct predictions from $0.5$ to $0.635$. For a KNN classifier with predictors $Lag1$ and $Lag2$ and $K = 375$, the fraction of correct predictions was $0.644$.
        ...
        matrix_of_values_of_predictors_for_training <- matrix(
            cbind(
                Weekly_from_1990_to_2008_inclusive$Lag1,
                Weekly_from_1990_to_2008_inclusive$Lag2
            ),
            nrow = 985,
            ncol = 2
        )
        matrix_of_values_of_predictors_for_testing <- matrix(
            cbind(
                Weekly_from_2009_to_2010_inclusive$Lag1,
                Weekly_from_2009_to_2010_inclusive$Lag2
            ),
            nrow = 104,
            ncol = 2
        )
        ...
        Through our experiments we discovered that our KNN models with optimized $K$ had highest fractions of correct predictions of $0.644$ and $0.635$. LR and LDA models with formula $Direction \sim Lag2$ had fractions of correct predictions of $0.625$ based on confusion matrices presented in parts (d) and (e).
