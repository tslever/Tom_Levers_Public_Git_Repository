...
7.  In this problem, you will use support vector approaches in order to predict whether a given car gets high or low gas mileage based on the `Auto` data set.
    ...
    (b) Fit a support vector classifier to the data with various values of `cost`, in order to predict whether a car gets high or low gas mileage. Report the cross-validation errors associated with different values of this parameter. Comment on your results.
        ...
        ```{r, eval = TRUE}
        set.seed(1)
        formula_as_string <- paste(
            "indicator_of_whether_gas_mileage_is_high ~ ",
            "cylinders + ",
            "displacement + ",
            "horsepower + ",
            "weight + ",
        ...
        tune_for_linear_SVM <- e1071::tune(
            METHOD = e1071::svm,
            train.x = formula,
            data = Auto,
            kernel = "linear",
        ...
        summary(tune_for_linear_SVM)
        ```
        
        A support vector classifier / support vector machine with linear kernel with a cost $C = 0.562$ has a lowest $10$-fold cross validation error rate of $0.0843$. This is supported by the below plot of error rate vs. cost for our SVM with linear kernel.
    ...
    (c) Now repeat (b), this time using SVM's with radial and polynomial basis kernels, with different values of `gamma` and `degree` and `cost.` Comment on your results.
        ...
        tune_for_polynomial_SVM <- e1071::tune(
            METHOD = e1071::svm,
            train.x = formula,
            data = Auto,
            kernel = "polynomial",
            ranges = list(
                cost = range_of_costs,
                degree = range_of_degrees
            )
        )
        summary(tune_for_polynomial_SVM)
        ```
        
        A support vector machine with polynomial kernel of degree $d = 3$ with a cost $C = 17.783$ has a lowest $10$-fold cross validation error rate of $0.0741$ percent. This is supported by the below plot of error rate vs. cost for our SVM with polynomial kernel.
        
        ```{r, eval = TRUE}
        set.seed(1)
        range_of_values_of_gamma <- 10^seq(-1, 1, by = 0.5)
        tune_for_radial_SVM <- e1071::tune(
            METHOD = e1071::svm,
            train.x = formula,
            data = Auto,
            kernel = "radial",
            ranges = list(
                cost = range_of_costs,
                gamma = range_of_values_of_gamma
            )
        )
        summary(tune_for_radial_SVM)
        ```
        
        A support vector machine with radial kernel with constant $\gamma = 1$ and cost $C = 1$ has a lowest $10$-fold cross validation error rate of $0.0663$ percent. This is supported by the below plot of error rate vs. cost for our SVM with radial kernel.
    ...
    (d) Make some plots to back up your assertions in (b) and (c).
        ...
        data_frame <- data.frame(
            cost = tune_for_linear_SVM$performances$cost,
            error = tune_for_linear_SVM$performances$error
        )
        library(ggplot2)
        ggplot(
            data = data_frame,
        ...
        As above, minimum error rate for our SVM with linear kernel occurs for cost $C = 10^{-0.25}$.
        
        ```{r}
        data_frame <- data.frame(
            cost = tune_for_polynomial_SVM$performances$cost,
            degree = tune_for_polynomial_SVM$performances$degree,
            error = tune_for_polynomial_SVM$performances$error
        )
        library(ggplot2)
        ggplot(
            data = data_frame,
        ...
        As above, minimum error rate for our SVM with polynomial kernel occurs for cost $C = 17.783$ and degree $d = 3$.
        
        ```{r}
        data_frame <- data.frame(
            cost = tune_for_radial_SVM$performances$cost,
            gamma = tune_for_radial_SVM$performances$gamma,
            error = tune_for_radial_SVM$performances$error
        )
        library(ggplot2)
        ggplot(
            data = data_frame,
        ...
        As above, minimum error rate for our SVM with radial kernel occurs for cost $C = 1$ and $\gamma = 1$.
        
        ```{r}
        minimum_acceleration <- min(Auto$acceleration)
        maximum_acceleration <- max(Auto$acceleration)
        median_number_of_cylinders <- median(Auto$cylinders)
        median_displacement <- median(Auto$displacement)
        median_year <- median(Auto$year)
        median_indicator_of_origin <- median(Auto$origin)
        sequence_of_accelerations <- seq(
            from = minimum_acceleration,
            to = maximum_acceleration,
            length.out = 4
        )
        for (acceleration in sequence_of_accelerations) {
            plot(
                x = tune_for_linear_SVM$best.model,
                data = Auto,
                formula = horsepower ~ weight,
                slice = list(
                    cylinders = median_number_of_cylinders,
                    displacement = median_displacement,
                    acceleration = acceleration,
                    year = median_year,
                    origin = median_indicator_of_origin
                )
            )
            plot(
                x = tune_for_polynomial_SVM$best.model,
                data = Auto,
                formula = horsepower ~ weight,
                slice = list(
                    cylinders = median_number_of_cylinders,
            ...
            plot(
                x = tune_for_radial_SVM$best.model,
                data = Auto,
                formula = horsepower ~ weight,
                slice = list(
                    cylinders = median_number_of_cylinders,
        ...
        SVM's with linear and radial kernels separate training points well from the perspective of a graph of horsepower vs. weight where acceleration is equal to its minimum and other predictors are equal to their medians. Radial SVM performs better than linear SVM. Polynomial SVM performs poorly. As acceleration increases, performance of SVM's generally decrease. Linear boundaries increasingly fail to capture actual positives. Radial boundaries increasingly fail to capture actual positives and become less circular. Polynomial boundaries improve by wrapping more tightly around the actual positives but then increasingly fail to capture actual positives.
