...
```{r, eval = FALSE, fig.cap = "Distribution Of Intensities Of Color Red", fig.width = 2.625, fig.height = 1.875}
library(TomLeversRPackage)
plot_distribution(training_data_frame_of_classes_and_pixels, "Red", "red")
```

```{r, eval = FALSE, fig.cap = "Distribution Of Intensities Of Color Green", fig.width = 2.625, fig.height = 1.875}
plot_distribution(training_data_frame_of_classes_and_pixels, "Green", "green")
```

```{r, eval = FALSE, fig.cap = "Distribution Of Intensities Of Color Blue", fig.width = 2.625, fig.height = 1.875}
...
```{r, eval = FALSE, fig.width = 2.625, fig.height = 1.875}
...
```{r, eval = FALSE, fig.cap = "Prediction Metrics Vs. Thresholds For Logistic Regression Model With Formula Indicator vs. Normalized Square Of Blue + Normalized Square Of Red + Normalized Square Root Of Blue"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = FALSE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For Logistic Regression Model With Formula Indicator vs. Normalized Square Of Blue + Normalized Square Of Red + Normalized Square Root Of Blue"}
...
```{r, eval = FALSE, fig.cap = "Prediction Metrics Vs. Thresholds For Logistic Ridge Regression Model With Formula Indicator Vs. Normalized Natural Logarithm Of Blue + Normalized Square Root Of Red + Normalized Natural Logarithm Of Green + Normalized Natural Logarithm Of Red"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = FALSE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For Logistic Ridge Regression Model With Formula Indicator Vs. Normalized Natural Logarithm Of Blue + Normalized Square Root Of Red + Normalized Natural Logarithm Of Green + Normalized Natural Logarithm Of Red"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
```

To search for an optimal LDA classifier, we perform bidirectional selection as above. Our optimal LDA classifier has formula $Indicator \sim normalize(Blue) + normalize(Red^2) + normalize\left(Green^2\right)$. The maximum average F1 measure is $0.891$; the corresponding threshold is $0.69$.
...
```{r, eval = FALSE, fig.cap = "Prediction Metrics Vs. Thresholds For Linear Discriminant Analysis Model With Formula Indicator Vs. Normalized Blue + Normalized Square Of Red + Normalized Square Of Green"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = FALSE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For Linear Discriminant Analysis Model With Formula Indicator Vs. Normalized Blue + Normalized Square Of Red + Normalized Square Of Green"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
```

To search for an optimal QDA classifier, we perform bidirectional selection as above. Our optimal QDA classifier has formula $Indicator \sim normalize\left(Blue\right) + normalize\left(Red^2\right) + normalize\left(Red:Blue\right)$. The maximum average F1 measure is $0.914$; the corresponding threshold is $0.19$.
...
```{r, eval = FALSE, fig.cap = "Prediction Metrics Vs. Thresholds For Quadratic Discriminant Analysis Model With Formula Indicator Vs. Normalized Blue + Normalized Square Of Red + Normalized Interaction Of Red And Blue"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = FALSE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For Quadratic Discriminant Analysis Model With Formula Indicator Vs. Normalized Blue + Normalized Square Of Red + Normalized Interaction Of Red And Blue"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
```

To search for an optimal KNN classifier, we perform bidirectional selection as above.

For a given formula, before cross validation, we search for one value of hyperparameter $K$. We use `caret::train` to choose $K = 1$ for all of our KNN classifiers. According to the [documentation for `caret::train`](https://www.rdocumentation.org/packages/caret/versions/4.47/topics/train), `caret::train` "sets up a grid of tuning parameters for a number of classification and regression routines, fits each model, and calculates a resampling based performance measure". Our inputs include our formula specifying response and predictive terms, our training data frame of indicators and pixels, the method "knn", the metric "F1_measure", a `trainControl` object, and a data frame containing one row for each value of $K$ to be evaluated. Dr. Gedeck recommended evaluating $K \in [1, 25]$. We construct the `trainControl` object with method "cv" denoting resampling method cross validation and summary function `calculate_F1_measure` to compute F1 measures across resamples. $K$ is a constant that determines the number of neighboring training observations used to determine the probability that a testing observation corresponds to a blue tarp.

See below graph of average F1 measure vs. $K$ for $10$ cross-validated KNN classifiers trained on the training data frame of indicators and pixels. `caret::train` chose the value of $K$ for all KNN binary classifiers corresponding to the maximum average F1 measure.
...
Average accuracy is nearly $1$ for all thresholds and is better than those for our optimal LR, LRR, LDA, and QDA classifiers. Average F1 measure is nearly $0.946$ for $93$ percent of thresholds; the maximum average F1 measure is higher than that for our optimal LR, LRR, LDA, and QDA classifiers. Average F1 measure, PPV, and TPR all jump to about $0.946$ for a threshold of about $0.08$. A Precision-Recall curve has an AUC of $0.948$. An ROC curve has an AUC of $0.978$. These AUC's are less than those for our optimal LR, LRR, and QDA classifiers but greater than that for our LDA classifiers.
...
```{r, eval = FALSE, fig.cap = "Prediction Metrics Vs. Thresholds For KNN Model With Formula Indicator Vs. Normalized Interaction Of Red And Blue + Normalized Interaction Of Green And Blue + Normalized Square Root Of Blue"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = FALSE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For KNN Model With Formula Indicator Vs. Normalized Interaction Of Red And Blue + Normalized Interaction Of Green And Blue + Normalized Square Root Of Blue"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
