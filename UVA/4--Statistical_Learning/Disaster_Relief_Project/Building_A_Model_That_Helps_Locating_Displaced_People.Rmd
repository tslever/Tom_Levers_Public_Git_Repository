---
title: "Building A Binary Classifier That Helps Locating Displaced People"
author: "Tom Lever"
date: 08/16/2023
output:
...
We conduct exploratory data analysis by considering the distributions of intensities of color $Red$, $Green$, and $Blue$ in our training data. The distribution of intensity of color $Red$ has a hill, some foothills, and a relatively high proportion of high intensities. The distributions of intensity of colors $Green$ and $Blue$ each have two hills and are not normal.
...
Our optimal SVMWRK has formula $Indicator \sim normalize(Blue) + normalize[\ln(Red)] + normalize\left(\sqrt{Green}\right) + normalize[\ln(Blue)]$.
...
![Misclassification Error Rate Vs. $\lambda$](Misclassification_Error_Rate_Vs_lambda.png){#id .class height=25%}
![F1 Measure Vs. Number Of Neighbors](F1_Measure_Vs_Number_Of_Neighbors.png){#id .class height=25%}
...
![Test Error Rate Vs. ntree For Different Values Of mtry](Test_Error_Rate_Vs_ntree_For_Different_Values_Of_mtry.png){#id .class height=25%}
![F1 Measure Vs. C](F1_Measure_Vs_C.png){#id .class height=25%}
...
![F1 Measure Vs. $d$ For Different Values Of $C$](F1_Measure_Vs_d_For_Different_Values_Of_C.png){#id .class height=25%}
![F1 Measure Vs. $\gamma$ For Different Values Of $C$](F1_Measure_Vs_gamma_For_Different_Values_Of_C.png){#id .class height=25%}

#### Support-Vector Machines With Radial Kernels

During bidirectional selection of SVMWRK's, for each formula, we search for one value of hyperparameter $C$ and one value of hyperparameter $\gamma$ to be used in training a SVMWRK on $9$ cross-validation folds. For our optimal formula, we use `caret::train` to choose $C = 10$ and $\gamma = 10$ for all of our SVMWPK's. Our inputs include our formula, our training data, the method "svmPoly", the metric "F1_measure", a `trainControl` object, and a data frame containing one row for each value of $C$ and each value of $d$ to be evaluated. Dr. Gedeck recommended evaluating $C \in \left\{10^{-3}, 10^{-2.75}, \cdots, 10^{2.75}, 10^3\right\}$ and $\gamma \in \{10^{-1}, 10^{-0.5}, 10^0, 10^{0.5}, 10^1\}$. Given time limits, we choose $C \in \left\{10^{-1}, 10^0, 10^1\right\}$ and $\gamma \in \{10^{-1}, 10^0, 10^1\}$.
...
![Average Performance Metrics Vs. Threshold: Cross Validation: LR](Average_Performance_Metrics_Vs_Threshold--Cross_Validation--LR.png){#id .class height=25%}
![Optimal Performance Metrics: Cross Validation: LR](Optimal_Performance_Metrics--Cross_Validation--LR.png){#id .class height=25%}

![PR And ROC Curves: Cross Validation: LR](PR_And_ROC_Curves--Cross_Validation--LR.png)

![Performance Metrics Vs. Threshold: Holdout Testing: LR](Performance_Metrics_Vs_Threshold--Holdout_Testing--LR.png){#id .class height=25%}
![Optimal Performance Metrics: Holdout Testing: LR](Optimal_Performance_Metrics--Holdout_Testing--LR.png){#id .class height=12%}

![PR And ROC Curves: Holdout Testing: LR](PR_And_ROC_Curves--Holdout_Testing--LR.png)
...
```{r, eval = FALSE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold"}
...
```{r, eval = FALSE}
list_of_visualizations_of_performance_metrics <- visualize_performance_metrics(
    formula = formula,
    testing_data = holdout_data_frame_of_indicators_and_pixels,
    training_data = training_data_frame_of_indicators_and_pixels,
    type_of_model = type_of_model,
    list_of_hyperparameters = NULL
)
list_of_visualizations_of_performance_metrics$plot_of_performance_metrics_vs_threshold
```

```{r, eval = FALSE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold"}
list_of_visualizations_of_performance_metrics$PR_curve
list_of_visualizations_of_performance_metrics$ROC_curve
t(list_of_visualizations_of_performance_metrics$data_frame_of_optimal_performance_metrics)
```

#### Logistic Ridge Regression Classifiers

Per LRR cross validation:
...
![Average Performance Metrics Vs. Threshold: Cross Validation: LRR](Average_Performance_Metrics_Vs_Threshold--Cross_Validation--LRR.png){#id .class height=25%}
![Optimal Performance Metrics: Cross Validation: LRR](Optimal_Performance_Metrics--Cross_Validation--LRR.png){#id .class height=25%}

![PR And ROC Curves: Cross Validation: LRR](PR_And_ROC_Curves--Cross_Validation--LRR.png)

![Performance Metrics Vs. Threshold: Holdout Testing: LRR](Performance_Metrics_Vs_Threshold--Holdout_Testing--LRR.png){#id .class height=25%}
![Optimal Performance Metrics: Holdout Testing: LRR](Optimal_Performance_Metrics--Holdout_Testing--LRR.png){#id .class height=12%}

![PR And ROC Curves: Holdout Testing: LRR](PR_And_ROC_Curves--Holdout_Testing--LRR.png)
...
list_of_visualizations_of_performance_metrics <- visualize_performance_metrics(
    ...
    training_data = training_data_frame_of_indicators_and_pixels,
    type_of_model = type_of_model,
    list_of_hyperparameters = list(lambda = 0.00012340980408668)
)
list_of_visualizations_of_performance_metrics$plot_of_performance_metrics_vs_threshold
```
...
![Average Performance Metrics Vs. Threshold: Cross Validation: LDA](Average_Performance_Metrics_Vs_Threshold--Cross_Validation--LDA.png){#id .class height=25%}
![Optimal Performance Metrics: Cross Validation: LDA](Optimal_Performance_Metrics--Cross_Validation--LDA.png){#id .class height=25%}

![PR And ROC Curves: Cross Validation: LDA](PR_And_ROC_Curves--Cross_Validation--LDA.png){#id .class height=19%}

![Performance Metrics Vs. Threshold: Holdout Testing: LDA](Performance_Metrics_Vs_Threshold--Holdout_Testing--LDA.png){#id .class height=25%}
![Optimal Performance Metrics: Holdout Testing: LDA](Optimal_Performance_Metrics--Holdout_Testing--LDA.png){#id .class height=12%}

![PR And ROC Curves: Holdout Testing: LDA](PR_And_ROC_Curves--Holdout_Testing--LDA.png){#id .class height=19%}
...
![Average Performance Metrics Vs. Threshold: Cross Validation: QDA](Average_Performance_Metrics_Vs_Threshold--Cross_Validation--QDA.png){#id .class height=25%}
![Optimal Performance Metrics: Cross Validation: QDA](Optimal_Performance_Metrics--Cross_Validation--QDA.png){#id .class height=25%}

![PR And ROC Curves: Cross Validation: QDA](PR_And_ROC_Curves--Cross_Validation--QDA.png){#id .class height=19%}

![Performance Metrics Vs. Threshold: Holdout Testing: QDA](Performance_Metrics_Vs_Threshold--Holdout_Testing--QDA.png){#id .class height=25%}
![Optimal Performance Metrics: Holdout Testing: QDA](Optimal_Performance_Metrics--Holdout_Testing--QDA.png){#id .class height=12%}

![PR And ROC Curves: Holdout Testing: QDA](PR_And_ROC_Curves--Holdout_Testing--QDA.png){#id .class height=19%}
...
![Average Performance Metrics Vs. Threshold: Cross Validation: KNN](Average_Performance_Metrics_Vs_Threshold--Cross_Validation--KNN.png){#id .class height=25%}
![Optimal Performance Metrics: Cross Validation: KNN](Optimal_Performance_Metrics--Cross_Validation--KNN.png){#id .class height=25%}

![PR And ROC Curves: Cross Validation: KNN](PR_And_ROC_Curves--Cross_Validation--KNN.png){#id .class height=19%}

![Performance Metrics Vs. Threshold: Holdout Testing: KNN](Performance_Metrics_Vs_Threshold--Holdout_Testing--KNN.png){#id .class height=25%}
![Optimal Performance Metrics: Holdout Testing: KNN](Optimal_Performance_Metrics--Holdout_Testing--KNN.png){#id .class height=12%}

![PR And ROC Curves: Holdout Testing: KNN](PR_And_ROC_Curves--Holdout_Testing--KNN.png){#id .class height=19%}

```{r, eval = FALSE}
library(TomLeversRPackage)
type_of_model <- "KNN"
formula <- Indicator ~ Normalized_Interaction_Of_Red_And_Blue + Normalized_Red + Normalized_Natural_Logarithm_Of_Green
set.seed(1)
summary_of_performance <- summarize_performance_of_cross_validated_classifiers(
    type_of_model = type_of_model,
    formula = formula,
...
list_of_visualizations_of_performance_metrics <- visualize_performance_metrics(
    ...
    training_data = training_data_frame_of_indicators_and_pixels,
    type_of_model = type_of_model,
    list_of_hyperparameters = list(K = 3)
)
list_of_visualizations_of_performance_metrics$plot_of_performance_metrics_vs_threshold
```
...
![Average Performance Metrics Vs. Threshold: Cross Validation: RF](Average_Performance_Metrics_Vs_Threshold--Cross_Validation--RF.png){#id .class height=25%}
![Optimal Performance Metrics: Cross Validation: RF](Optimal_Performance_Metrics--Cross_Validation--RF.png){#id .class height=25%}

![PR And ROC Curves: Cross Validation: RF](PR_And_ROC_Curves--Cross_Validation--RF.png){#id .class height=19%}

![Performance Metrics Vs. Threshold: Holdout Testing: RF](Performance_Metrics_Vs_Threshold--Holdout_Testing--RF.png){#id .class height=25%}
![Optimal Performance Metrics: Holdout Testing: RF](Optimal_Performance_Metrics--Holdout_Testing--RF.png){#id .class height=12%}

![PR And ROC Curves: Holdout Testing: RF](PR_And_ROC_Curves--Holdout_Testing--RF.png){#id .class height=19%}
...
list_of_visualizations_of_performance_metrics <- visualize_performance_metrics(
    ...
    training_data = training_data_frame_of_indicators_and_pixels,
    type_of_model = type_of_model,
    list_of_hyperparameters = list(mtry = 1, ntree = 52)
)
list_of_visualizations_of_performance_metrics$plot_of_performance_metrics_vs_threshold
```
...
![Average Performance Metrics Vs. Threshold: Cross Validation: SVMWLK](Average_Performance_Metrics_Vs_Threshold--Cross_Validation--SVMWLK.png){#id .class height=25%}
![Optimal Performance Metrics: Cross Validation: SVMWLK](Optimal_Performance_Metrics--Cross_Validation--SVMWLK.png){#id .class height=25%}

![PR And ROC Curves: Cross Validation: SVMWLK](PR_And_ROC_Curves--Cross_Validation--SVMWLK.png){#id .class height=19%}

![Performance Metrics Vs. Threshold: Holdout Testing: SVMWLK](Performance_Metrics_Vs_Threshold--Holdout_Testing--SVMWLK.png){#id .class height=25%}
![Optimal Performance Metrics: Holdout Testing: SVMWLK](Optimal_Performance_Metrics--Holdout_Testing--SVMWLK.png){#id .class height=12%}

![PR And ROC Curves: Holdout Testing: SVMWLK](PR_And_ROC_Curves--Holdout_Testing--SVMWLK.png){#id .class height=19%}
...
list_of_visualizations_of_performance_metrics <- visualize_performance_metrics(
    ...
    training_data = training_data_frame_of_indicators_and_pixels,
    type_of_model = type_of_model,
    list_of_hyperparameters = list(cost = 10)
)
list_of_visualizations_of_performance_metrics$plot_of_performance_metrics_vs_threshold
```
...
![Average Performance Metrics Vs. Threshold: Cross Validation: SVMWPK](Average_Performance_Metrics_Vs_Threshold--Cross_Validation--SVMWPK.png){#id .class height=25%}
![Optimal Performance Metrics: Cross Validation: SVMWPK](Optimal_Performance_Metrics--Cross_Validation--SVMWPK.png){#id .class height=25%}

![PR And ROC Curves: Cross Validation: SVMWPK](PR_And_ROC_Curves--Cross_Validation--SVMWPK.png){#id .class height=19%}

![Performance Metrics Vs. Threshold: Holdout Testing: SVMWPK](Performance_Metrics_Vs_Threshold--Holdout_Testing--SVMWPK.png){#id .class height=25%}
![Optimal Performance Metrics: Holdout Testing: SVMWPK](Optimal_Performance_Metrics--Holdout_Testing--SVMWPK.png){#id .class height=12%}

![PR And ROC Curves: Holdout Testing: SVMWPK](PR_And_ROC_Curves--Holdout_Testing--SVMWPK.png){#id .class height=19%}

```{r, eval = FALSE}
library(TomLeversRPackage)
type_of_model <- "Support-Vector Machine With Polynomial Kernel"
formula <- Indicator ~ Normalized_Square_Of_Blue + Normalized_Square_Of_Red + Normalized_Interaction_Of_Green_And_Blue
set.seed(1)
summary_of_performance <- summarize_performance_of_cross_validated_classifiers(
    type_of_model = type_of_model,
    formula = formula,
...
list_of_visualizations_of_performance_metrics <- visualize_performance_metrics(
    ...
    training_data = training_data_frame_of_indicators_and_pixels,
    type_of_model = type_of_model,
    list_of_hyperparameters = list(cost = 10, degree = 3)
)
list_of_visualizations_of_performance_metrics$plot_of_performance_metrics_vs_threshold
```
...
![Average Performance Metrics Vs. Threshold: Cross Validation: SVMWRK](Average_Performance_Metrics_Vs_Threshold--Cross_Validation--SVMWRK.png){#id .class height=25%}
![Optimal Performance Metrics: Cross Validation: SVMWRK](Optimal_Performance_Metrics--Cross_Validation--SVMWRK.png){#id .class height=25%}

![PR And ROC Curves: Cross Validation: SVMWRK](PR_And_ROC_Curves--Cross_Validation--SVMWRK.png){#id .class height=19%}

![Performance Metrics Vs. Threshold: Holdout Testing: SVMWPK](Performance_Metrics_Vs_Threshold--Holdout_Testing--SVMWRK.png){#id .class height=25%}
![Optimal Performance Metrics: Holdout Testing: SVMWPK](Optimal_Performance_Metrics--Holdout_Testing--SVMWRK.png){#id .class height=12%}

![PR And ROC Curves: Holdout Testing: SVMWPK](PR_And_ROC_Curves--Holdout_Testing--SVMWRK.png){#id .class height=19%}

```{r, eval = FALSE}
library(TomLeversRPackage)
type_of_model <- "Support-Vector Machine With Radial Kernel"
formula <- Indicator ~ Normalized_Blue + Normalized_Natural_Logarithm_Of_Red + Normalized_Square_Root_Of_Green + Normalized_Natural_Logarithm_Of_Blue
set.seed(1)
summary_of_performance <- summarize_performance_of_cross_validated_classifiers(
    type_of_model = type_of_model,
    formula = formula,
...
list_of_visualizations_of_performance_metrics <- visualize_performance_metrics(
    ...
    training_data = training_data_frame_of_indicators_and_pixels,
    type_of_model = type_of_model,
    list_of_hyperparameters = list(cost = 10, gamma = 10)
)
list_of_visualizations_of_performance_metrics$plot_of_performance_metrics_vs_threshold
```
...
Please see [Cross-Validation And Holdout Performance Tables](https://github.com/tslever/Tom_Levers_Public_Git_Repository/tree/main/UVA/4--Statistical_Learning/Disaster_Relief_Project).
...
Our optimal LRR classifier with formula $Indicator \sim normalize[\ln(Blue)] + normalize(\sqrt{Red}) + normalize\left[\ln(Green)\right] + normalize[\ln(Red)]$, $\lambda = 0.000123$, and $t = 0.18$ has highest holdout binary-classifier score. Our optimal SVMWLK has second-highest holdout binary-classifier score. Our optimal LRR classifier has for holdout highest PR AUC, FPR, PPV, and F1 measure and third-highest TPR. If we were to recommend a classifier based on holdout binary-classifier score, or holdout F1 measure, we would recommend our optimal LRR classifier.

Our SVMWRK has highest cross-validation binary classifier score by $2$ but by far the worst holdout binary-classifier score, suggesting overfitting, and the second-worst holdout binary-classifier score. Our optimal RF with formula $Indicator \sim normalize(Red^2) + normalize(Red:Green) + normalize\left(Green:Blue\right) + normalize(Green)$, $mtry = 1$, $ntree = 52$, and $t = 0.39$ has the second-highest cross-validation binary-classifier score. If we were to recommend a classifier without knowledge of our holdout data based on cross-validation binary-classifier score, or cross-validation F1 measure, we would recommend our SVMWRK. If we were to recommend a classifier with knowledge of our holdout data based on cross-validation binary-classifier score, or cross-validation F1 measure, we would recommend our RF.

![Cross-Validation And Holdout Performance Tables](Cross-Validation_And_Holdout_Performance_Tables.png)
