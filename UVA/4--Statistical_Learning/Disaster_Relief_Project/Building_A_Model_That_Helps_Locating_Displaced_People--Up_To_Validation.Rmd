...
```{r, eval = TRUE, fig.cap = "Distribution Of Intensities Of Colors In Training Data", fig.width = 7, fig.height = 2.5}
library(gridExtra)
...
![Reconstruction Of Image Of Region 57](./Holdout_Data/Region_57.png){#id .class height=25%}
![Reconstruction Of Image Of Region 67](./Holdout_Data/Region_67.png){#id .class height=25%}
![Reconstruction Of Image Of Region 69](./Holdout_Data/Region_69.png){#id .class height=25%}
![Reconstruction Of Image Of Region 78](./Holdout_Data/Region_78.png){#id .class height=25%}

We conduct exploratory data analysis by considering the distributions of intensities of color $Red$, $Green$, and $Blue$ in our holdout data frame of classes and pixels. The distributions of intensity of colors $Red$, $Green$ and $Blue$ are normal and broad relative to our training distributions, except for long right tails and relatively high proportions of high intensities.

```{r, eval = TRUE, fig.width = 2.625, fig.height = 1.875}
...
```{r, eval = TRUE, fig.cap = "Distribution Of Classes In Intensity Space", echo = FALSE}
distribution_of_classes_in_intensity_space <- plotly::plot_ly(
...
The fourth dimension of our grid is value of secondary hyperparameter. For RF classifiers, the secondary hyperparameter is $ntree$, the number of trees that we want to create. According to Dr. Basener, "And what is the effect of this parameter? So the more trees [you] have the more accuracy you get, but up to a point. Typically, you'll get a lot of increase when you go from $1$ to $2$ to $10$ or $20$ trees, and then you'll increase more up to $100$. But at some point, increasing doesn't give you any more accuracy. So that may be at $500$ that you don't want to create any more trees, or $2000$ you have enough trees. It depends a lot of how complicated of a decision you need to make, how complicated your space is, how many features you have, how much data you have." According to Dr. Gedeck, "a good rule of thumb is that usually from $100$ trees onward, not much happens."

For SVM classifiers, the secondary hyperparameter is degree $d$ for classifiers with polynomial kernels and $\gamma$ for classifiers with radial kernels. According to *An Introduction to Statistical Learning*, "It can be shown that... The linear support vector classifier can be represented as
...
As described below, we apply cross validation to averaging performance metrics and choosing $\lambda$ for LRR classifiers, $K$ for KNN classifiers, and $C$ and $\gamma$ for SVM classifiers. We apply cross validation to averaging performance metrics using `rsample::vfold_cv`. We use `vfold_cv` by passing `vfold_cv` our training data frame of indicators and pixels, and specifications of numbers of partitions of the data set and times to repeat $v$-fold partitioning. We apply cross validation to choosing $\lambda$, $K$, and $C$ and $\gamma$ using `caret::train` by passing `train` a `caret::trainControl` constructed with `method = "cv"`.

### Bidirectional Selection

To search for an optimal LR classifier, we perform bidirectional selection. For each formula, we perform $10$-fold cross validation. For each fold, we record $100$ thresholds and corresponding performance metrics including F1 measure. For each formula, we record $100$ thresholds and corresponding average performance metrics. For each formula, we find the maximum average F1 measure. For each formula, we choose the threshold that yields the highest maximum average F1 measure. We choose the LR classifier with the formula and threshold with the highest maximum average F1 measure.

To describe bidirectional selection in detail, we determine the maximum average F1 measure for LR classifiers with formula $Indicator \sim normalize(Red)$. Similarly, we calculate the maximum average F1 measure for LR classifiers with formula $Indicator \sim normalize(Green), Indicator \sim normalize(Blue), ..., Indicator \sim normalize\left(Green : Blue\right)$. The classifiers with formula $Indicator \sim normalize(Blue^2)$ have the highest maximum average F1 measure of $0.446$. We consider the maximum average F1 measures for LR classifiers with formulas $Indicator \sim normalize(Blue^2) + normalize(Red), ..., Indicator \sim normalize(Blue^2) + normalize(Green : Blue)$. The classifiers with formula $Indicator \sim normalize(Blue^2) + normalize(Red^2)$ have the highest maximum average F1 measure of $0.912$. Since we have already considered the maximum average F1 measures for classifiers with formulas $Indicator \sim normalize(Red^2)$ and $Indicator \sim normalize(Blue^2)$, we do not drop the other predictor and do not consider the F1 measures of these classifiers. We consider the maximum average F1 measures for classifiers with formulas $Indicator \sim normalize(Blue^2) + normalize(Red^2) + normalize(Red), ..., Indicator \sim normalize(Blue^2) + normalize(Red^2) + normalize(Green:Blue)$. The classifiers with formula $Indicator \sim normalize(Blue^2) + normalize(Red^2) + normalize\left(\sqrt{Blue}\right)$ have the highest maximum average F1 measure of $0.937$. We drop predictor $normalize(Blue^2)$ and consider the maximum average F1 measure for classifiers with formula $Indicator \sim normalize(Red^2) + normalize\left(\sqrt{Blue}\right)$. These classifiers have a maximum average F1 measure of $0.903$. We choose a LR classifier with formula $Indicator \sim normalize(Blue^2) + normalize(Red^2) + normalize\left(\sqrt{Blue}\right)$.

Our optimal LR classifier has formula $Indicator \sim normalize(Blue^2) + normalize(Red^2) + normalize\left(\sqrt{Blue}\right)$.

To search for an optimal LRR classifier, we perform bidirectional selection as above. Unfortunately, bidirectional selection from predictor $normalize(Blue^2)$ results ultimately classifiers with formula $Indicator ~ normalize(Blue^2) + normalize(Green:Blue) + normalize(\ln(Blue))$ and maximum average F1 measure $0.447$. We conduct an exhaustive search for optimal classifiers with a formula with two predictors; the optimal classifiers have formula $Indicator ~ normalize(\ln(Blue)) + normalize(\sqrt(Red)) + normalize(\ln(Green)) + normalize(\ln(Red))$. We perform bidirectional selection using this formula as a baseline. Our optimal LRR classifier has formula $Indicator \sim normalize(\ln(Blue)) + normalize(\sqrt{Red}) + normalize\left(\ln(Green)\right) + normalize(\ln(Red))$.

Our optimal LDA classifier has formula $Indicator \sim normalize(Blue) + normalize(Red^2) + normalize\left(Green^2\right)$.

To search for an optimal LDA classifier, we perform bidirectional selection as above. Our optimal LDA classifier has formula $Indicator \sim normalize(Blue) + normalize(Red^2) + normalize\left(Green^2\right)$.

To search for an optimal QDA classifier, we perform bidirectional selection as above. Our optimal QDA classifier has formula $Indicator \sim normalize\left(Blue\right) + normalize\left(Red^2\right) + normalize\left(Red:Blue\right)$.

To search for an optimal KNN classifier, we perform bidirectional selection as above. Our optimal $KNN$ binary classifier has formula $Indicator \sim normalize(Red:Blue) + normalize(Red) + normalize\left(\ln(Green)\right)$.

Our optimal $RF$ classifier has formula $Indicator \sim normalize(Red^2) + normalize(Red:Green) + normalize\left(Green:Blue\right) + normalize(Green)$.

### Tuning Hyperparameters

#### Logistic Ridge Regression Classifiers

During bidirectional selection of LRR classifiers, for each formula, we search for one value of hyperparameter $\lambda$ to be used in training a LRR classifier on $k - 1$ cross-validation folds. For our optimal formula, we use `caret::train` to choose $\lambda = 0.000123$. According to the [documentation for `caret::train`](https://www.rdocumentation.org/packages/caret/versions/4.47/topics/train), `caret::train` "sets up a grid of tuning parameters for a number of classification and regression routines, fits each model, and calculates a resampling based performance measure". Our inputs include our formula specifying response and predictive terms, our training data frame of indicators and pixels, the method "glmnet", the metric "F1_measure", a `trainControl` object, and a data frame containing one row for each combination of $\alpha = 0$ and value of $\lambda$ to be evaluated. We construct the `trainControl` object with method "cv" denoting resampling method Cross Validation and summary function `calculate_F1_measure` to compute F1 measures across resamples. $\alpha = 0$ indicates a combination of ridge regression and lasso regression that is effectively ridge regression.
...
During bidirectional selection of KNN classifiers, for each formula, we search for one value of hyperparameter $K$ to be used in training a KNN classifier on $k - 1$ cross-validation folds. For our optimal formula, we use `caret::train` to choose $K = 1$ for all of our KNN classifiers. Our inputs include our formula specifying response and predictive terms, our training data frame of indicators and pixels, the method "knn", the metric "F1_measure", a `trainControl` object, and a data frame containing one row for each value of $K$ to be evaluated. Dr. Gedeck recommended evaluating $K \in [1, 25]$.

See below graph, for our optimal formula, of average F1 measure vs. $K$ for $10$ cross-validated KNN classifiers. `caret::train` chose the value of $K$ corresponding to the maximum average F1 measure. This value of $K$ will be used in training a KNN classifier on $k - 1$ cross-validation folds.

#### Random Forest Classifiers

During bidirectional selection of RF classifiers, for each formula, we search for one value of hyperparameter $mtry$ and one value of hyperparameter $ntree$ to be used in training an RF classifier on $k - 1$ cross-validation folds. We create plots of average test error rate vs. number of trees for different values of $mtry$. Values of $mtry$ vary from $1$ to the number of predictors in the formula. Number of trees varies from $1$ to $500$. We choose the values of $mtry$ and $ntree$ corresponding to the minimum test error rate.

#### Support-Vector Machines With Linear Kernels

During bidirectional selection of Support-Vector Machines With Linear Kernels, for each formula, we search for one value of hyperparameter $cost$ to be used in training a SVMWLK on $k - 1$ cross-validation folds. For our optimal formula, we use `caret::train` to choose $cost = 10$ for all of our SVMWLK classifiers.

During bidirectional selection of SVMWLK classifiers, for each formula, we search for one value of hyperparameter $cost$ to be used in training a SVMWLK classifier on $k - 1$ cross-validation folds. For our optimal formula, we use `caret::train` to choose $cost = 10$ for all of our SVMWLK classifiers. Our inputs include our formula specifying response and predictive terms, our training data frame of indicators and pixels, the method "svmLinear", the metric "F1_measure", a `trainControl` object, and a data frame containing one row for each value of $K$ to be evaluated. Dr. Gedeck recommended evaluating $cost \in \left\{10^{-3}, 10^{-2.75}, \cdots, 10^{2.75}, 10^3\right\}$. Given time, we choose $cost \in \left\{10^{-1}, 10^0, 10^1\right\}$.

See below graph, for our optimal formula, of average F1 measure vs. $cost$ for $10$ cross-validated SVMWLK classifiers. `caret::train` chose the value of $cost$ corresponding to the maximum average F1 measure. This value of $cost$ will be used in training an SVMWLK classifier on $k - 1$ cross-validation folds.

### Evaluating and Comparing Cross-Validated Classifiers

For each type of model, optimal formula, optimal primary hyperparameter where applicable, and optimal secondary hyperparameter where applicable, we provide a plot of Average Performance Metrics Vs. Threshold, a Precision-Recall Curve, an ROC curve, and a data frame of optimal performance metrics.

Our optimal LR classifier has formula $Indicator \sim normalize(Blue^2) + normalize(Red^2) + normalize\left(\sqrt{Blue}\right)$. The maximum average F1 measure is $0.937$; the corresponding threshold is $0.25$.
...
```{r, eval = TRUE, fig.cap = "Prediction Metrics Vs. Thresholds For Logistic Regression Model With Formula Indicator vs. Normalized Square Of Blue + Normalized Square Of Red + Normalized Square Root Of Blue"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = TRUE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For Logistic Regression Model With Formula Indicator vs. Normalized Square Of Blue + Normalized Square Of Red + Normalized Square Root Of Blue"}
...
```{r, eval = TRUE, fig.cap = "Prediction Metrics Vs. Thresholds For Logistic Ridge Regression Model With Formula Indicator Vs. Normalized Natural Logarithm Of Blue + Normalized Square Root Of Red + Normalized Natural Logarithm Of Green + Normalized Natural Logarithm Of Red"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = TRUE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For Logistic Ridge Regression Model With Formula Indicator Vs. Normalized Natural Logarithm Of Blue + Normalized Square Root Of Red + Normalized Natural Logarithm Of Green + Normalized Natural Logarithm Of Red"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
data_frame_of_fold_ids_and_maximum_F1_measures_for_LRR <- 
    summary_of_performance$data_frame_of_fold_ids_and_maximum_F1_measures
```

Our optimal LDA classifier has formula $Indicator \sim normalize(Blue) + normalize(Red^2) + normalize\left(Green^2\right)$. The maximum average F1 measure is $0.891$; the corresponding threshold is $0.69$.

Average accuracy is nearly $1$ for about $25$ percent of thresholds and in general is not quite as good as that for our LR classifier or that for our optimal LRR classifier. Average F1 measure is nearly $0.891$ for maybe $6$ percent of thresholds; the maximum average F1 measure is lower than those for our optimal LR and LRR classifiers. The maximum average F1 measure is about $0.891$ for a smaller proportion of thresholds than the similar proportions of thresholds for our optimal LR and LRR classifiers. Average PPV increases logarithmically from a threshold of about $0$ to our optimal threshold of $0.69$, at which point average PPV jumps up to $1$. From about a threshold of $0.69$ PPV is greater than that for our optimal LR classifier and equal to that for our optimal LRR classifier. For thresholds lower than our optimal threshold of $0.69$ PPV is less than those for our optimal LR and LRR classifiers. Average TPR generally has a similar trend and is less than that for our optimal LR classifier. Average TPR is generally less than that for our optimal LRR classifier for thresholds less than $0.43$ and greater than that for our optimal LRR classifier for thresholds greater than $0.43$. We prioritize TPR over precision but under F1 measure. A Precision-Recall curve has an AUC of $0.904$. An ROC curve has an AUC of $0.949$. These AUC's are less than those for our optimal LR and LRR classifiers.
...
```{r, eval = TRUE, fig.cap = "Prediction Metrics Vs. Thresholds For Linear Discriminant Analysis Model With Formula Indicator Vs. Normalized Blue + Normalized Square Of Red + Normalized Square Of Green"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = TRUE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For Linear Discriminant Analysis Model With Formula Indicator Vs. Normalized Blue + Normalized Square Of Red + Normalized Square Of Green"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
data_frame_of_fold_ids_and_maximum_F1_measures_for_LDA <- 
    summary_of_performance$data_frame_of_fold_ids_and_maximum_F1_measures
```

Our optimal QDA classifier has formula $Indicator \sim normalize\left(Blue\right) + normalize\left(Red^2\right) + normalize\left(Red:Blue\right)$. The maximum average F1 measure is $0.914$; the corresponding threshold is $0.19$.

Average accuracy is nearly $1$ for about $90$ percent of thresholds but in general is not quite as good as that for our optimal LR classifiers. Average accuracy is better than that for our optimal LRR and LDA classifiers. Average F1 measure is nearly $0.914$ for 58 percent of thresholds; the maximum average F1 measure is lower than that for our optimal LR and LRR classifiers but higher than that for our optimal LDA classifiers. Average PPV increases logarithmically. In general PPV is greater than that for our optimal LR classifiers. PPV is greater than that for our optimal LRR classifiers for thresholds less than $0.38$ and less than that for our optimal LRR classifiers for thresholds greater than $0.38. PPV is greater than that for our optimal LDA classifiers for thresholds less than $0.69$ and less than that for our optimal LDA classifiers for thresholds greater than $0.69$. Average TPR decreases in the manner of $y = -tan(x)$. In general TPR is less than that for our LR classifiers and greater than that for our LRR and LDA classifiers. We prioritize TPR over precision but under F1 measure. A Precision-Recall curve has an AUC of $0.959$. An ROC curve has an AUC of $0.984$. These AUC's are less than those for our optimal LR and LRR classifiers but greater than that for our optimal LDA classifiers.

```{r, eval = TRUE}
summary_of_performance <- summarize_performance_of_cross_validated_classifiers(
    type_of_model = "QDA",
    formula = Indicator ~ Normalized_Blue + Normalized_Square_Of_Red + Normalized_Interaction_Of_Red_And_Blue,
    data_frame = training_data_frame_of_indicators_and_pixels
)
```

```{r, eval = TRUE, fig.cap = "Prediction Metrics Vs. Thresholds For Quadratic Discriminant Analysis Model With Formula Indicator Vs. Normalized Blue + Normalized Square Of Red + Normalized Interaction Of Red And Blue"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = TRUE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For Quadratic Discriminant Analysis Model With Formula Indicator Vs. Normalized Blue + Normalized Square Of Red + Normalized Interaction Of Red And Blue"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
data_frame_of_fold_ids_and_maximum_F1_measures_for_QDA <- 
    summary_of_performance$data_frame_of_fold_ids_and_maximum_F1_measures
```

Our optimal $KNN$ binary classifier has formula $Indicator \sim normalize(Red:Blue) + normalize(Red) + normalize\left(\ln(Green)\right)$. Our optimal value for $K$ is $1$; the maximum average F1 measure is $0.946$; the corresponding threshold is $0.5$.

Average accuracy is nearly $1$ for all thresholds and is better than those for our optimal LR, LRR, LDA, and QDA classifiers. Average F1 measure is nearly $0.946$ for $93$ percent of thresholds; the maximum average F1 measure is higher than those for our optimal LR, LRR, LDA, and QDA classifiers. Average F1 measure, PPV, and TPR all jump to about $0.946$ for a threshold of about $0.08$. A Precision-Recall curve has an AUC of $0.948$. An ROC curve has an AUC of $0.978$. These AUC's are less than those for our optimal LR, LRR, and QDA classifiers but greater than that for our LDA classifiers.
...
```{r, eval = TRUE, fig.cap = "Prediction Metrics Vs. Thresholds For KNN Model With Formula Indicator Vs. Normalized Interaction Of Red And Blue + Normalized Interaction Of Green And Blue + Normalized Square Root Of Blue"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = TRUE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For KNN Model With Formula Indicator Vs. Normalized Interaction Of Red And Blue + Normalized Interaction Of Green And Blue + Normalized Square Root Of Blue"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
...
Our optimal $RF$ classifier has formula $Indicator \sim normalize(Red^2) + normalize(Red:Green) + normalize\left(Green:Blue\right) + normalize(Green)$. Our optimal values for $mtry$ and $ntree$ are $2$ and $47$; the maximum average F1 measure is $0.954$; the corresponding threshold is $0.38$.

Average accuracy is nearly $1$ for all thresholds and is better than those for our optimal LR, LRR, LDA, QDA, and KNN classifiers. Average F1 measure is nearly $0.954$ for $35$ percent of thresholds; the maximum average F1 measure is higher than that for our optimal LR, LRR, LDA, QDA, and KNN classifiers. Average F1 measure is bow shaped and generally higher than for our optimal LR, LRR, LDA, QDA, and KNN classifiers. PPV and TPR increase and decrease logarithmically respectively. PPV is generally higher than that for our optimal LR and LDA classifiers but lower than that for our optimal LRR, QDA, and KNN classifiers. TPR is generally higher than that for our optimal LR, LRR, LDA, QDA, and KNN classifiers. A Precision-Recall curve has an AUC of $0.986$. An ROC curve has an AUC of $0.994$. The PR AUC is greater than those for our optimal LR, LRR, LDA, QDA, and KNN classifiers. The ROC AUC is greater than those for our LDA, QDA, and KNN classifiers but less than those for our LR and LRR classifiers.

```{r, eval = TRUE}
summary_of_performance <- summarize_performance_of_cross_validated_classifiers(
    type_of_model = "Random Forest",
    formula = Indicator ~ Normalized_Square_Of_Red + Normalized_Interaction_Of_Red_And_Green + Normalized_Interaction_Of_Green_And_Blue + Normalized_Green,
    data_frame = training_data_frame_of_indicators_and_pixels
)
```

```{r, eval = TRUE, fig.cap = "Prediction Metrics Vs. Thresholds For RF"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = TRUE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For RF"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
data_frame_of_fold_ids_and_maximum_F1_measures_for_RF <- 
    summary_of_performance$data_frame_of_fold_ids_and_maximum_F1_measures
```

Our optimal Support-Vector Machine with linear kernel has formula $Indicator \sim normalize(Red) + normalize(Green^2) + normalize\left(Blue\right) + normalize(Blue^2) + normalize(\sqrt{Red}) + normalize(\sqrt{Blue})$. Our optimal value for cost $C = 10$; the maximum average F1 measure is $0.946$; the corresponding threshold is $0.15$.

Average accuracy is nearly $1$ for $95$ percent of thresholds and is better than those for our optimal LRR, LDA, and QDA classifiers, worse than those for our optimal KNN and RF classifiers, and about the same for that for our optimal LR classifiers. Average F1 measure is nearly $0.946$ for $20$ percent of thresholds; the maximum average F1 measure is higher than those for our optimal LR, LRR, LDA, QDA classifiers and lower than those for our optimal KNN and RF classifiers. Average F1 measure is bow shaped. PPV increases logarithmically. TPR decreases linearly. A Precision-Recall curve has an AUC of $0.984$. An ROC curve has an AUC of $0.998$. The PR AUC is greater than those for our optimal LR, LRR, LDA, QDA, and KNN classifiers and less than that for our optimal RF classifiers. The ROC AUC is greater than those for our LDA, QDA, KNN, and RF classifiers but less than those for our LR and LRR classifiers.

PPV is generally higher than that for our optimal LR and LDA classifiers but lower than that for our optimal LRR, QDA, and KNN classifiers. TPR is generally higher than that for our optimal LR, LRR, LDA, QDA, and KNN classifiers.
...
```{r, eval = TRUE, fig.cap = "Prediction Metrics Vs. Thresholds For Support-Vector Machine With Linear Kernel With Formula Indicator Vs. Normalized Red And Normalized Square Of Green And Normalized Blue And Normalized Square Of Blue And Normalized Square Root Of Red And Normalized Square Root Of Blue"}
summary_of_performance$plot_of_performance_metrics_vs_threshold
```

```{r, eval = TRUE, fig.width = 2.625, fig.height = 1.875, fig.show = "hold", fig.cap = "PR And ROC Curves For Support-Vector Machine With Linear Kernel With Formula Indicator Vs. Normalized Red And Normalized Square Of Green And Normalized Blue And Normalized Square Of Blue And Normalized Square Root Of Red And Normalized Square Root Of Blue"}
summary_of_performance$PR_curve
summary_of_performance$ROC_curve
t(summary_of_performance$data_frame_of_optimal_performance_metrics)
data_frame_of_fold_ids_and_maximum_F1_measures_for_SVMWLK <- 
    summary_of_performance$data_frame_of_fold_ids_and_maximum_F1_measures
```

## Validation / Holdout Test

An optimal classifier for a given type of classifier is a classifier that yields a high F1 measure relative to classifiers for that type. An optimal classifier has certain formula, primary hyperparameter where applicable, and secondary hyperparameter where applicable. We have determined optimal LR, LRR, LDA, QDA, KNN, RF, and SVMWLK classifiers according to cross validation on our training data frame with maximum average F1 measure greater than $0.89$.

That being said, the number of observations in our training data frame, $63,241$, is $3.2$ percent of the number of observations in our holdout data frame, $2,004,177$. Our holdout distributions of intensity of colors $Red$, $Green$ and $Blue$ are normal and broad relative to our training distributions. The proportion of training observations that correspond to blue tarps is about $25$ times the proportion of holdout observations that correspond to blue tarps. We expect that, due to bias, an optimal classifier validated on our holdout data frame will have a maximum F1 measure that is less than its maximum average F1 measure across cross-validation folds. While we recommend cross validating on our composite data frame or $90$ percent of our composite data frame, we have trained on our training data frame and will validate our optimal classifiers on our holdout data frame. Validation will provide us performance metrics for a larger, perhaps more realistic data frame and ideas of the robustness of our optimal classifiers across data frames.

To validate our optimal classifiers, we train each optimal classifier on our training data and visualize performance metrics on our holdout data.

Stuff about each classifier's validation visualizations
Comparisons of validations
Each classifier's validation visualizations

# Results

## Cross-Validation Performance Table

Please see "Method Of Determination" in our Cross-Validation Performance Table for descriptions of how performance metrics ere calculated under our cross-validation framework.

Per our [Cross-Validation Performance Table](https://github.com/tslever/Tom_Levers_Public_Git_Repository/tree/main/UVA/4--Statistical_Learning/Disaster_Relief_Project), our KNN binary classifier with $K = 1$ and threshold $t = 0.080$ has the best optimal F1 measure and corresponding accuracy. Our KNN binary classifier has the second-best corresponding TPR, FPR, and PPV. However, our KNN binary classifier has the second-worst Area Under The Precision-Recall Curve and Area Under The ROC Curve. Our Logistic Regression binary classifier has the best Area Under The Precision-Recall Curve, Area Under The ROC Curve, and corresponding TPR. Our Logistic Regression binary classifier has the second-best corresponding optimal measure and corresponding accuracy. However, our Logistic Regression binary classifier has the worst corresponding FPR and PPV. Beautifully, our QDA binary classifier is completely middle of the road. Our Logistic Ridge Regression binary classifier is second-best in Area Under The Precision-Recall Curve and Area Under The ROC curve and second-worst in everything else. Our LDA binary classifier has the best corresponding FPR and PPV and is worst in everything else.
...
![Cross-Validation Performance Table](Cross-Validation_Performance_Table.png)

## Validation / Holdout Test Performance Table

DISCUSS AND DISPLAY VALIDATION PERFORMANCE TABLE

## Comparing And Contrasting Cross-Validation and Holdout Test

Compare and contrast tuning parameters
Compare and contrast AUC's
Compare and contrast optimal thresholds
Compare and contrast accuracy, TPR, FPR, and precision corresponding to optimal threshold
...
Results may improve by hybridizing our Logistic Ridge Regression binary classifier with a Logistic Lasso Regression binary classifier by specifying $alpha$ to be greater than $0$ and less than or equal to $1$. Results may improve by developing software that can find the minimum in a relatively continuous plot of F1 measure Vs. $log(\lambda)$. Results may improve by considering values of $K$ between $1$ and the number of observations in our training data frame of indicators and pixels $n$.
