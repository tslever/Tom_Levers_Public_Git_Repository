...
8.  We will now perform cross-validation on a simulated data set.
    ...
    (a) Generate a simulated data set as follows:
        ...
        The number of observations $n = 100$. The number of predictors $p = 1$. The number of predictive terms is $2$. The model used to generate the data
    ...
    (c) Set a random seed, and then compute the LOOCV errors that result from fitting the following four models using least squares:
        ...
        i.  $Y = \beta_0 + \beta_1X + \epsilon$
            ...
            calculate_LOOCV_error_rate(
                type_of_model = "Polynomial Regression",
                formula = y ~ x,
                data_frame = data_frame,
                number_of_folds = number_of_observations
            )
            ```
    ...
    (f) Comment on the statistical significance of the coefficient estimates that results from fitting each of the models in (c) using least squares. Do these results agree with the conclusions drawn based on the cross-validation results?
        ...
        ```{r}
        polynomial_model <- glm(formula = y ~ x, data = data_frame)
        summary(polynomial_model)$coefficients
        polynomial_model <- glm(formula = y ~ x + I(x^2), data = data_frame)
        summary(polynomial_model)$coefficients
        polynomial_model <- glm(formula = y ~ x + I(x^2) + I(x^3), data = data_frame)
        summary(polynomial_model)$coefficients
        polynomial_model <- glm(formula = y ~ x + I(x^2) + I(x^3) + I(x^4), data = data_frame)
        summary(polynomial_model)$coefficients
        ```
